...
describe('DesktopNavigation > open', () => {
  it('Contains 200 width when Nav is Open', async () => {
    const wrapper = render(<DesktopNavigation navData={navMock} />);

    const iconClose = wrapper.container.querySelector(
  ...
  it('Contains Logo', async () => {
    const wrapper = render(<DesktopNavigation navData={navMock} />);
    const toggleButton = wrapper.container.querySelector(
      '[data-test="toggleButton"]'
    ) as HTMLButtonElement;
  ...
  it('Contains closed icon', async () => {
    ...
    const iconClosed = wrapper.container.querySelector(
      '[data-test="closedIcon"]'
    );
