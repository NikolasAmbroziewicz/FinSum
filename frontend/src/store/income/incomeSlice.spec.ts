...
import { addIncome, getIncome, editIncome, deleteIncome } from './IncomeSlice'
...
describe('incomeSlice > addIncome', () => {
  ...
  it('Add Income to store',async () => {
    ...
    //than
    mock.onPost(`http://localhost:8080/income/v1/add-income`).reply(200, mockAddIncome);

    await testStore.dispatch(addIncome(mockAddIncome))

    // then
    expect(testStore.getState().income.income[0]).toMatchObject(mockAddIncomeResponse);
  })
  ...
  it('Do not add Income to store', async () => {
    ...
    mock.onPost(`http://localhost:8080/income/v1/add-income`).networkErrorOnce();
...
describe('incomeSlice > getIncome', () => {
  ...
  it('Get Income from store', async () => {
    ...
    mock.onGet(`http://localhost:8080/income/v1/get-income?date=${new Date('2020')}`).reply(200, mockGetIncome);

    await testStore.dispatch(getIncome(new Date('2020')))

    // then
    expect(testStore.getState().income.income[0]).toMatchObject(mockGetIncome[0]);
  })

  it('Do not get Income from store', async () => {
    mock.onGet(`http://localhost:8080/income/v1/get-income?date=${new Date('2020')}`).networkErrorOnce();
...
describe('incomeSlice > editIncome', () => {
  ...
  it('Get edited Income from Store', async () => {
    ...
    //then
    mock.onPut(`http://localhost:8080/income/v1/edit-income?id=123`).reply(200, mockEditIncomeResponse);

    expect(testStore.getState().income.income[0]).toMatchObject(storeValue)

    await testStore.dispatch(editIncome(mockEditIncome))

    expect(testStore.getState().income.income[0]).toMatchObject(mockEditIncomeResponse);
  })

  it('Do not get edited Income from store', async () => {
    //given
    const testStore = setupStore()
    mock.onPut('http://localhost:8080/income/v1/edit-income?id=123').networkErrorOnce();
    const mockAddIncome = {
      title: 'Test',
      currency: "USD",
      amount: "123",
...
describe('incomeSlice > deleteIncome', () => {
  ...
  it('Should delete income from store', async () => {
    ...
    mock.onDelete('http://localhost:8080/income/v1/delete-income?id=123').reply(200, storeValue)
