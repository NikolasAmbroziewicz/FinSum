...
import { userRegister, userResponseFromDataBase, userLogin, userWithToken } from './mocks'
...
const generateTokens = (
  ...
  const tokens = {
    accessToken: tokenService.createRefreshToken(userWithToken.id, userWithToken.email, accessTokenTime),
    refreshToken: tokenService.createRefreshToken(userWithToken.id, userWithToken.email, configService.get<string>('REFRESH_TOKEN_TIME_TO_LIVE'))
  }
...
beforeEach(async () => {
  const app: TestingModule = await Test.createTestingModule({
    providers: [AuthService, TokensService, ConfigService,
      {
...
describe('AuthService > methods > signIn', () => {
  ...
  it('should throw error when email don\'t exist', async () => {
    ...
    await expect(service.signIn(userLogin)).rejects.toEqual(new ForbiddenException('User doesn\'t exist'))
  })
