...
import { userWithToken, addedIncomeInput, addedIncome, editIncome, editIncomeInput } from './mocks'
...
describe('IncomeController > methods > addIncome', () => {
  ...
  it('should return income', async () => {
    ...
    const controllerMethod = await controller.addIncome(addedIncomeInput, userWithToken)
...
describe('IncomeController > methods > deleteIncome', () => {
  ...
  it('Should throw error when income does not exist', async () => {
    deleteIncomeMock.mockImplementation(() => new ForbiddenException('Income does not exist'))
...
describe('IncomeController > methods > editIncome', () => {
  ...
  it('Should return error when income does not exist', async () => {
    editIncomeMock.mockImplementation(() => new ForbiddenException('Income does not exist'))

    const controllerMethod = await controller.editIncome(editIncomeInput ,8) as any
